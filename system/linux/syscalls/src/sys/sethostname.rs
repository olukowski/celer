...
///   earlier NUL byte was seen. Linux 1.0 rejected `len > 64` but did not
///   contain an entry-path check for negative `len`.
...
///   64-byte hostname limit.
/// - `EINVAL`: on Linux 1.0, `len` exceeds the kernel's 64-byte hostname
///   limit.
...
mod tests {
    ...
    fn test_sethostname_negative_length_is_rejected_or_permission_denied() {
        ...
        assert!(
            ret == -1 || ret == -22,
            "expected EPERM or EINVAL from sethostname(len = -1), got {ret}",
        );
    }
}
